        ...
        ServiceResponse.forResult(
          LinkDeviceProgressImpl(
            "sgnl://linkdevice?uuid=" +
              URLEncoder.encode(accountManager.getNewDeviceUuid(), "utf-8") +
              "&pub_key=" +
              URLEncoder.encode(Base64.encodeBytesWithoutPadding(tempIdentityKey.getPublicKey().getPublicKey().serialize()), "utf-8"),
            tempIdentityKey,
            accountManager,
            registrationData,
            deviceName
  ...
  fun attemptDeviceLink(
    ...
    return Single.fromCallable<ServiceResponse<LinkDeviceResponse>> {
      try {
        ...
        val encryptedDeviceName = deviceName?.let { DeviceNameUtil.encryptDeviceName(it, ret.getAciIdentity().getPrivateKey()) }
        val deviceId = accountManager.finishNewDeviceRegistration(
          ret.getProvisioningCode(),
          ConfirmCodeMessage(
            false,
            true,
