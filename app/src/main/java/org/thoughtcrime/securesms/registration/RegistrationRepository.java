...
public final class RegistrationRepository {
    ...
    return Single.<ServiceResponse<SignalServiceAccountManager.NewDeviceRegistrationReturn>>fromCallable(() -> {
      try {
        ...
        registerAccountInternal(new RegistrationData(registrationData.getCode(),
                                ...
                                response.getAci(), response.getPni(), false, deviceId, deviceName, null, null, false);
      ...
      throws IOException
  {
    ...
    registerAccountInternal(registrationData, ACI.parseOrThrow(response.getUuid()), PNI.parseOrThrow(response.getPni()), response.isStorageCapable(), SignalServiceAddress.DEFAULT_DEVICE_ID, null, pin, kbsData, setRegistrationLockEnabled);
  }
      ...
      throws IOException
  {
    ...
    SignalServiceAccountManager       accountManager   = AccountManagerFactory.getInstance().createAuthenticated(context, aci, pni, registrationData.getE164(), deviceId, registrationData.getPassword());
